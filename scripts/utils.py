...
from settings import max_tokens, stop_text_generation_on, bot_repeat_penalty, chat_history_limit, gpu_layers, tensor_split
...
async def start_up(current_model):
    ...
    command = f"python3 -m llama_cpp.server --model {current_model} --host 0.0.0.0 --port 8000 --n_gpu_layers {gpu_layers} --split-mode --tensor_split {tensor_split}"
...
def change_model(model):
    ...
    command = f"python3 -m llama_cpp.server --model {model} --host 0.0.0.0 --port 8000 --n_gpu_layers {gpu_layers} --split-mode --tensor_split {tensor_split}"
    subprocess.Popen(command, shell=True)   
